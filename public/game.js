...
// ─── CREATURE SVG ART ────────────────────────────────────────────────────────
const CREATURE_SVGS = {
  luminos: `<svg viewBox="0 0 120 120" xmlns="http://www.w3.org/2000/svg">
  <defs>
  ...
  pyradon: `<svg viewBox="0 0 120 120" xmlns="http://www.w3.org/2000/svg">
  <defs>
  ...
  aqualis: `<svg viewBox="0 0 120 120" xmlns="http://www.w3.org/2000/svg">
  <defs>
  ...
  sylvara: `<svg viewBox="0 0 120 120" xmlns="http://www.w3.org/2000/svg">
  <defs>
  ...
  voltex: `<svg viewBox="0 0 120 120" xmlns="http://www.w3.org/2000/svg">
  <defs>
  ...
  shadowfang: `<svg viewBox="0 0 120 120" xmlns="http://www.w3.org/2000/svg">
  <defs>
  ...
  frostelle: `<svg viewBox="0 0 120 120" xmlns="http://www.w3.org/2000/svg">
  <defs>
  ...
  terrarok: `<svg viewBox="0 0 120 120" xmlns="http://www.w3.org/2000/svg">
  <defs>
...
const Game = {
  ...
  generateOfflineCreatures(lat, lng) {
    const ids = ['luminos','pyradon','aqualis','sylvara','voltex','shadowfang','frostelle','terrarok'];
    const creatures = [];
    for (let i = 0; i < 12; i++) {
      creatures.push({
        id: 'off_' + i,
  ...
  addCreatureMarker(creature) {
    ...
    const svg = CREATURE_SVGS[def.id] || CREATURE_SVGS.terrarok;
  ...
  showEncounter(creature, def) {
    ...
    document.getElementById('enc-svg').innerHTML = CREATURE_SVGS[def.id] || CREATURE_SVGS.terrarok;
  ...
  startOfflineBattle(creatureId) {
    ...
    const playerDef = State.creatureDefs.find(d => d.id === 'terrarok') || State.creatureDefs[0];
  ...
  showBattle(battle) {
    ...
    document.getElementById('b-wild-svg').innerHTML = CREATURE_SVGS[battle.wildDef.id] || CREATURE_SVGS.terrarok;
    document.getElementById('b-player-svg').innerHTML = CREATURE_SVGS[battle.playerDef.id] || CREATURE_SVGS.terrarok;
  ...
  renderCollection() {
    ...
    grid.innerHTML = coll.map(c => {
      ...
      return `<div class="coll-card" style="--card-glow:${def.colors?.primary||'#888'}">
        <div class="coll-card-svg">${CREATURE_SVGS[def.id] || CREATURE_SVGS.terrarok}</div>
  ...
  renderProfile() {
    ...
    grid.innerHTML = State.creatureDefs.map(def => {
      ...
      return `<div class="ndex-card ${have ? '' : 'locked'}">
        <div class="ndex-svg">${CREATURE_SVGS[def.id] || CREATURE_SVGS.terrarok}</div>
